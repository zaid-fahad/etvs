...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    async function fetchUser() {
      try {
        ...
        if (!profile || profile.role !== "club") return (window.location.href = "/auth/error");

        setUser({
          email: data.user.email,
          full_name: profile.full_name || "Club User",
          role: profile.role,
        });
      } catch (err) {
        console.error(err);
  ...
  const menuItems = [
    { href: "/club", icon: faTachometerAlt, label: "Dashboard" },
    // { href: "/admin/clubs", icon: faUsers, label: "Clubs" },
    { href: "/club/event-proposals", icon: faFileAlt, label: "Proposals" },
    { href: "/club/events", icon: faCalendarCheck, label: "Events" },
  ];
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 w-64 bg-cyan-950 text-white shadow-lg transform transition-transform duration-300 z-40 flex flex-col",
      ...
      >
        <div>
          <div className="p-4 flex items-center space-x-3 border-b border-indigo-700">
            ...
            <h1 className="text-xl font-bold truncate">Club Portal</h1>
          </div>
